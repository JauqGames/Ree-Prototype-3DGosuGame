	...
	def update
		if @load == 1.0
			...
			@stonearray.each do |stone|
				...
				@solidtilemap.each do |tile|
					if a_is_right_of_b(stone, tile)
						if a_hits_right_of_b(stone, tile)
							if stone.prehit != "right"
								if tile.canhit_r == 1
									stone.prehit = "n"
									stone.xhit
								end
					...
					if a_is_left_of_b(stone, tile)
						if a_hits_left_of_b(stone, tile)
							if stone.prehit != "left"
								if tile.canhit_l == 1
									stone.prehit = "left"
									stone.xhit
								end
					...
					if a_is_below_b(stone, tile)
						if a_hits_bottom_of_b(stone, tile)
							if stone.prehit != "bottom"
								if tile.canhit_b == 1
									stone.prehit = "bottom"
									stone.yhit
								end
...
class Player
	attr_accessor :x, :y, :width, :height, :facing, :jumped, :moving, :leftcoll, :rightcoll, :bottcoll, :topcoll, :yvelocity, :sprite, :below, :aimdir, :aimdeg, :aimcharge
	def initialize(x, y)
		@x = x * 16
		@y = y * 16
		@prex = x
...
class Stone
	...
	def draw
		@image.draw(@x - 0 - $xcamera, @y - 0 - $ycamera, 5)
	end
end
