...
class GameWindow < Gosu::Window
	...
	def place_platform(x, y, width, id, condition, reverse = false)					#additive
		if condition == 0
	...
	def update
		if @load == 1.0
			...
			place_platform(12, 17, 4, 40, 1, true)
			...
			@conditionalplatformtilemap.each do |tile|
				if tile.x + tile.width + 20 > $xcamera and tile.x - 20 < $xcamera + 196 and tile.y + tile.height + 20 > $ycamera and tile.y - 20 < $ycamera + 160
					if $conditions[tile.condition] > 0 and tile.reverse == false
						platform_tile_collide_player(tile)
					elsif $conditions[tile.condition] < 1 and tile.reverse == true
						platform_tile_collide_player(tile)
					end
				end
			end
...
class ConditionalTile
	...
	def draw(playerx, playery)
		...
		if $debugmode == true
			@debug.draw("#{@reverse}", @x - $xcamera, @y - $ycamera, 7)
		end
	end
