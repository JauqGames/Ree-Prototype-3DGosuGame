...
class GameWindow < Gosu::Window
	...
	def update
		if @load == 1.0
			...
			if @solid_format_count == @solidtilemap.count and @decor_format_count == @decortilemap.count
				@load == 2.0
				@showload = false
			end
...
class Enemy
	attr_accessor :id, :x, :y, :width, :height, :stretchlimit, :health, :xvelocity, :yvelocity, :bottcoll, :topcoll, :rightcoll, :leftcoll, :hurt
