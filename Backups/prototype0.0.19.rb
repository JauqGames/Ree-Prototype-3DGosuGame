	...
	def update
		if @load == 1.0
			...
			@conditionalplatformtilemap.each do |tile|
				if $conditions[tile.condition] > 0 and tile.x + tile.width + 20 > $xcamera and tile.x - 20 < $xcamera + 196 and tile.y + tile.height + 20 > $ycamera and tile.y - 20 < $ycamera + 160
					platform_tile_collide_player(tile)
				end
			end
...
class Enemy
	attr_accessor :id, :x, :y, :width, :height, :stretchlimit, :health, :xvelocity, :yvelocity, :bottcoll, :topcoll, :rightcoll, :leftcoll
	...
	def draw(playerx, playery)
		...
		#@debug.draw(@rightcoll, @x - $xcamera, @y - $ycamera, 25)
		#@debug.draw(@leftcoll, @x - $xcamera, @y - $ycamera + 6, 25)
		#@debug.draw(@bottcoll, @x - $xcamera, @y - $ycamera + 12, 25)
		#@debug.draw(@topcoll, @x - $xcamera, @y - $ycamera + 18, 25)
	end
end
