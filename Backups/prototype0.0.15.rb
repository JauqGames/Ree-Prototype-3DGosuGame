	...
	def update
		if @load == 1.0
			...
			@solidtilemap.each do |tile|
				...
				#solid_tile_collide_float(enemy, tile)
				end
			end
			@platformtilemap.each do |tile|
			...
			@enemyarray.each do |enemy|
				enemy.ai(@player.x, @player.y, @player.width, @player.height)
			end
...
class Enemy
	...
	def detectplayer(playerx, playery, playerwidth, playerheight, width, height)
		playerx + playerwidth > @x - width and playerx < @x + @width + width and playery + playerheight > @y - height and playery < @y + @height + height	
	end
	
	def ai(playerx, playery, playerwidth, playerheight)
		if @id == 0
			if @xvelocity > 1.5
				@xvelocity = 1.5
			elsif @xvelocity < -1.5
			...
			@y += @yvelocity
			if @active == 0
				if detectplayer(playerx, playery, playerwidth, playerheight, 110, 90)
					@active = 1
				else
					@active = 0
				end
			elsif @active == 1
				if @x > playerx + (playerwidth / 2)
					if @x > playerx + (playerwidth / 2) + @personality
						@xvelocity -= 0.1
						@facing = "left"
					elsif @x == playerx + (playerwidth / 2) + @personality
						if @xvelocity > 0
							@xvelocity -= 0.1
							@facing = "left"
						elsif @xvelocity < 0
				...
				if @y > playery + playerheight
					@yvelocity -= 0.1
				elsif @y + @height < playery
					@yvelocity += 0.1
				else
					if @yvelocity > 0
						@yvelocity -= 0.1
