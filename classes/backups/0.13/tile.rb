class Tile
	...
	def initialize(x, y, id, rid, lid, tid, bid, nid, alwaysdraw, write, tiles)
		@x = x * 16
		@y = y * 16
		@width = 16
		@height = 16
	...
	def draw(playerx, playery)
		...
		if $debugmode == true
			@debug.draw("#{@write}", @x - $xcamera, @y - $ycamera, 35 + @p - @aalayerx - @aalayery)
		end
	end
end