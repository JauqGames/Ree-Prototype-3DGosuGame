	require 'gosu'
	classarray = ["breakable_tile", "button", "conditional_tile", "crosshair", "decortile", "enemy", "faketiles", "hazard_tile", "hpd", "particle", "player", "portrait", "projectile", "stone", "tile", "tutorial", "visual_gem"]
	
	classarray.count.times do |z|
		require_relative "classes/#{classarray[z]}"
	end
...
class GameWindow < Gosu::Window
	...
	def initialize()
		...
		@tut_bools = [false, false, false, 0]
		@tut_tools = []
		@tut_tools.push(Gosu::Image.new('img\leftright.png'))
		@tut_tools.push(Gosu::Image.new('img\jump.png'))
		@tut_tools.push(Gosu::Image.new('img\sling.png'))
		@tut_tools.push(Gosu::Image.new('img\shoot.png'))
		@tut_tools.push(Gosu::Image.new('img\switch.png'))
		@tutorial = Tutorial.new(@tut_tools)
		...
		@die_image = Gosu::Image.load_tiles('img\die.png', 16, 16)
		@die_sprite = 0
		@dsp = 0
		@dspmax = 5
	...
	def pdec(x, y, id, animlength, stretchlimit, layer, tiles = $tilesimage)
	...
	def enemy(x, y, id, spritemap = nil, window = self)
		spritemap = Gosu::Image.load_tiles('img\enemyspritemap.png', 16, 16)
		@enemyarray.push(Enemy.new(x, y, id, spritemap, window))
	end
	def projectile(x, y, xv, yv, id, aimdeg = 0, map = nil)
		map = Gosu::Image.load_tiles('img\particlemap.png', 8, 8)
		@projectilearray.push(Projectile.new(x, y, xv, yv, id, aimdeg, map))
	end
...
=end			
	...
	def update
		if @load == 1.0
			...
			pdec(21, 17, 34, 1, 8, 3)
			pdec(24, 16, 34, 1, 8, 3)
			pdec(24, 17, 35, 1, 10, 3)
			pdec(23, 17, 32, 1, 10, 3)
			pdec(22, 17, 31, 1, 10, 3)
			pdec(21, 17, 34, 1, 10, 3)		
			pdec(24, 17, 32, 1, 17, 4)
			pdec(23, 17, 31, 1, 17, 4)
			pdec(22, 17, 34, 1, 17, 4)
			pdec(8, 17, 34, 1, 7, 3)
			pdec(9, 17, 31, 1, 7, 3)
			pdec(10, 17, 33, 1, 7, 3)
			pdec(37, 27, 32, 1, 7, 3)
			...
			25.times do |z|
				if z >= 0 and z <= 3
					pdec(3 + (4 * z), 3, 21, 3, 7, 3)
				elsif z >= 4 and z <= 9
					pdec(3 + (4 * (z - 4)), 15, 21, 3, 7, 3)
				elsif z >= 10 and z <= 17
					pdec(3 + (4 * (z - 10)), 26, 21, 3, 7, 3)
				elsif z >= 18 and z <= 20
					pdec(32 + (4 * (z - 18)), 15, 21, 3, 7, 3)
				elsif z >= 21 and z <= 24
					pdec(42 + (4 * (z - 21)), 23, 21, 3, 7, 3)
				end
			end
			...
			place_platform(95, 35, 2, 40, 5)
			place_platform(92, 37, 5, 40, 6)
			...
			83.times do |z|
				if z >= 0 and z <= 10
					pdec(42 + (4 * z), 45, 21, 3, 7, 3)
				elsif z >= 11 and z <= 21
					pdec(52 + (4 * (z - 11)), 33, 21, 3, 7, 3)
				elsif z >= 22 and z <= 24
					pdec(86 + (4 * (z - 22)), 42, 21, 3, 7, 3)
				elsif z >= 25 and z <= 27
					pdec(86 + (4 * (z - 25)), 38, 21, 3, 7, 3)
				elsif z >= 28 and z <= 31
					pdec(2 + (4 * (z - 28)), 33, 21, 3, 7, 3)
				elsif z >= 32 and z <= 38
					pdec(18 + (4 * (z - 32)), 34, 21, 3, 7, 3)
				elsif z >= 39 and z <= 41
					pdec(6 + (4 * (z - 39)), 38, 21, 3, 7, 3)
				elsif z >= 42 and z <= 46
					pdec(26 + (4 * (z - 44)), 39, 21, 3, 7, 3)
				elsif z >= 47 and z <= 53
					pdec(1 + (4 * (z - 47)), 18, 21, 3, 7, 3)
				elsif z >= 54 and z <= 57
					pdec(31 + (4 * (z - 54)), 18, 21, 3, 7, 3)
				elsif z >= 58 and z <= 62
					pdec(52 + (4 * (z - 58)), 18, 21, 3, 7, 3)
				elsif z >= 63 and z <= 69
					pdec(19 + (4 * (z - 63)), 11, 21, 3, 7, 3)
				elsif z >= 70 and z <= 74
					pdec(52 + (4 * (z - 70)), 11, 21, 3, 7, 3)
				elsif z >= 75 and z <= 78
					pdec(94, 19 + (4 * (z - 75)), 21, 3, 7, 3)
				elsif z >= 80 and z <= 82
					pdec(85 + (4 * (z - 80)), 15, 21, 3, 7, 3)
				end
			end
			3.times do |z|
				pdec(7, 22 + (z * 4), 21, 3, 7, 3)
				pdec(10, 22 + (z * 4), 21, 3, 7, 3)
			end
			
			#waterfalls
			55.times do |z|
				pdec(48, -7 + z, 51, 4, 7, 3)
			end
			3.times do |z|
				pdec(48, 48 + z, 51, 4, 7, 2)
			end
			pdec(48, 51, 51, 4, 7, 3)
			35.times do |z|
				pdec(75, 0 + z, 51, 4, 7, 3)
			end
			10.times do |z|
				pdec(75, 37 + z, 51, 4, 7, 3)
			end
			pdec(75, 35, 51, 4, 7, 2)
			pdec(75, 36, 51, 4, 7, 2)
			4.times do |z|
				pdec(75, 47 + z, 51, 4, 7, 2)
			end
			pdec(75, 51, 51, 4, 7, 3)

			#gold piles
				# ids: 				26 = solid deep gold, 30 = left side steep, 31 = shallow connection, 32 = right side steep, 33 = left taper, 34 = right taper, 35 = deep connection
				# layers: 			2 = behind regular plats, 3 = behind regular blocks, 4 = infront of player
				# stretchlimits:	7, 8, 10 = layer 2/3, 17, 34, 68 = layer 4
				# pdec(x, y, id, animlength, stretchlimit, layer, tiles = $tilesimage)
				if true		##	dark purple zone
					pdec(96, 43, 31, 1, 7, 3)
					pdec(95, 43, 34, 1, 7, 3)
					pdec(96, 44, 26, 1, 7, 3)
					pdec(95, 44, 26, 1, 7, 3)
					...
					pdec(96, 44, 32, 1, 10, 3)
					pdec(95, 44, 34, 1, 10, 3)
					pdec(94, 45, 32, 1, 10, 3)
					pdec(93, 45, 34, 1, 10, 3)
					pdec(94, 46, 26, 1, 10, 3)
					pdec(93, 46, 26, 1, 10, 3)
					pdec(92, 46, 32, 1, 10, 3)
					pdec(91, 46, 34, 1, 10, 3)
					pdec(89, 47, 32, 1, 10, 3)
					pdec(88, 47, 34, 1, 10, 3)

					pdec(96, 44, 34, 1, 17, 4)
					pdec(94, 46, 35, 1, 17, 4)
					pdec(93, 46, 32, 1, 17, 4)
					pdec(92, 46, 34, 1, 17, 4)
					pdec(89, 47, 34, 1, 17, 4)
					
					pdec(94, 46, 34, 1, 34, 4)
				end
				if true		##  dark green zone
					pdec(10, 38, 34, 1, 7, 3)
					pdec(11, 38, 31, 1, 7, 3)
					pdec(12, 38, 33, 1, 7, 3)
					pdec(7, 39, 34, 1, 7, 3)
					...
					pdec(6, 40, 35, 1, 7, 3)
					8.times do |z|
						pdec(7 + z, 40, 26, 1, 7, 3)
					end
					pdec(15, 40, 30, 1, 7, 3)
					pdec(16, 40, 33, 1, 7, 3)
					9.times do |z|
						pdec(8 + z, 41, 26, 1, 7, 3)
					end
					pdec(17, 41, 30, 1, 7, 3)
					pdec(18, 41, 31, 1, 7, 3)
					pdec(19, 41, 33, 1, 7, 3)
					...
					pdec(6, 40, 31, 1, 8, 3)
					5.times do |z|
						pdec(8 + z, 40, 26, 1, 8, 3)
					end
					pdec(14, 40, 30, 1, 8, 3)
					pdec(15, 40, 33, 1, 8, 3)
					8.times do |z|
						pdec(8 + z, 41, 26, 1, 8, 3)
					end
					pdec(16, 41, 30, 1, 8, 3)
					pdec(17, 41, 33, 1, 8, 3)
					
					pdec(10, 39, 34, 1, 10, 3)
					pdec(11, 39, 31, 1, 10, 3)
					pdec(12, 39, 33, 1, 10, 3)
					pdec(8, 40, 34, 1, 10, 3)
					pdec(9, 40, 32, 1, 10, 3)
					pdec(10, 40, 26, 1, 10, 3)
					pdec(11, 40, 26, 1, 10, 3)
					pdec(12, 40, 26, 1, 10, 3)
					pdec(13, 40, 30, 1, 10, 3)
					pdec(14, 40, 33, 1, 10, 3)
					7.times do |z|
						pdec(8 + z, 41, 26, 1, 10, 3)
					end
					pdec(15, 41, 30, 1, 10, 3)
					pdec(16, 41, 33, 1, 10, 3)
					
					pdec(10, 40, 34, 1, 17, 4)
					pdec(11, 40, 31, 1, 17, 4)
					pdec(12, 40, 31, 1, 17, 4)
					pdec(13, 40, 33, 1, 17, 4)
					pdec(8, 41, 31, 1, 17, 4)
					pdec(9, 41, 32, 1, 17, 4)
					4.times do |z|
						pdec(10 + z, 41, 26, 1, 17, 4)
					end
					pdec(14, 41, 30, 1, 17, 4)
					pdec(15, 41, 33, 1, 17, 4)
					
					pdec(9, 41, 34, 1, 34, 4)
					pdec(10, 41, 32, 1, 34, 4)
					pdec(11, 41, 35, 1, 34, 4)
					pdec(12, 41, 30, 1, 34, 4)
					pdec(13, 41, 33, 1, 34, 4)

					pdec(10, 41, 34, 1, 68, 4)
					pdec(11, 41, 31, 1, 68, 4)
					pdec(12, 41, 33, 1, 68, 4)					
				end
			...
			#TEST
			@visualgemarray.push(VisualGem.new(22, 45, 1, "type", Gosu::Image.load_tiles('img\smallgemsmap.png', 20, 20)))
			@visualgemarray.push(VisualGem.new(53, 39, 2, "type", Gosu::Image.load_tiles('img\smallgemsmap.png', 20, 20)))
			@visualgemarray.push(VisualGem.new(31, 26, 3, "type", Gosu::Image.load_tiles('img\smallgemsmap.png', 20, 20)))
			@visualgemarray.push(VisualGem.new(89, 15, 4, "type", Gosu::Image.load_tiles('img\smallgemsmap.png', 20, 20)))
			...
			83.times do |z|
				if z >= 0 and z <= 10
					replace_tile(42 + (4 * z), 45, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 11 and z <= 21
					replace_tile(52 + (4 * (z - 11)), 33, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 22 and z <= 24
					replace_tile(86 + (4 * (z - 22)), 42, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 25 and z <= 27
					replace_tile(86 + (4 * (z - 25)), 38, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 28 and z <= 31
					replace_tile(2 + (4 * (z - 28)), 33, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 32 and z <= 38
					replace_tile(18 + (4 * (z - 32)), 34, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 39 and z <= 41
					replace_tile(6 + (4 * (z - 39)), 38, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 42 and z <= 46
					replace_tile(26 + (4 * (z - 44)), 39, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 47 and z <= 53
					replace_tile(1 + (4 * (z - 47)), 18, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 54 and z <= 57
					replace_tile(31 + (4 * (z - 54)), 18, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 58 and z <= 62
					replace_tile(52 + (4 * (z - 58)), 18, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 63 and z <= 69
					replace_tile(19 + (4 * (z - 63)), 11, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 70 and z <= 74
					replace_tile(52 + (4 * (z - 70)), 11, 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 75 and z <= 78
					replace_tile(94, 19 + (4 * (z - 75)), 0, 0, 0, 0, 0, 24, false, "air")
				elsif z >= 80 and z <= 82
					replace_tile(85 + (4 * (z - 80)), 15, 0, 0, 0, 0, 0, 24, false, "air")
				end
			end
			3.times do |z|
				replace_tile(7, 22 + (z * 4), 0, 0, 0, 0, 0, 24, false, "air")
				replace_tile(10, 22 + (z * 4), 0, 0, 0, 0, 0, 24, false, "air")
			end			
			@load = 1.23
			...
			if  button_down?(Gosu::KbRight) and $playerhealth > 0
				@player.movement("right")
				if @tut_bools[0] == false
					@tut_bools[0] = true
				end
			elsif button_down?(Gosu::KbLeft) and $playerhealth > 0
				@player.movement("left")
				if @tut_bools[1] == false
					@tut_bools[1] = true
				end
			else
			...
			if button_down?(Gosu::KbUp) and $playerhealth > 0
				if @player.bottcoll == true and @player.jumped == false
					@player.jump
					if @tut_bools[2] == false
						@tut_bools[2] = true
					end
			...
			@visualgemarray.each do |gem|
				if a_is_inside_of_b(@player, gem)
					$gems[(gem.id / 11)] = 1
					@visualgemarray.delete(gem)
				end
			end
			...
			@hazardtilemap.each do |tile|
				if tile.x + tile.width + 20 > $xcamera and tile.x - 20 < $xcamera + 196 and tile.y + tile.height + 20 > $ycamera and tile.y - 20 < $ycamera + 160
					if a_is_inside_of_b(@player, tile)
						@player.hurt_player(5, @player.facing)
					end
				end
			end
			...
			@projectilearray.each do |proj|
				if proj.active
					proj.gravity
					if a_is_inside_of_b(@player, proj)
						if proj.x + proj.width > @player.x + @player.width
					...
					@solidtilemap.each do |tile|
						if tile.x + tile.width + 20 > proj.x and tile.x - 20 < proj.x and tile.y + tile.height + 20 > proj.y and tile.y - 20 < proj.y
							if a_is_inside_of_b(proj, tile)
								proj.active = false
							end
						end
					end
			...
			if @colorswitch_number >= 0
				if $gems[@colorswitch_number] == 1
					if @colorswitch_number == 0
						...
						$colorswitch = 0xff_2c0736
						$colorswitch2 = 0xff_ffffaf
						$colorswitch3 = 0xff_1c0726			
					elsif @colorswitch_number == 3
						$colorswitch = 0xff_00ff00
						$colorswitch2 = 0xff_000000
						$colorswitch3 = 0xff_008000
					elsif @colorswitch_number == 4
						$colorswitch = 0xff_00afaf
						$colorswitch2 = 0xff_afafaf
						$colorswitch3 = 0xff_af0000	
	...
	def draw												#draw
		if @showload == true
			@loadingbg.draw(0, 0, 40 + @p)
			@loadinganim[@la].draw(131, 143, 40 + @p)
			if @la < 5
				if @lacount == 1
					@lacount -= 1
					@la += 1
		...
		if $playerhealth > 0
			...
			if @player.facing == "right" and @die_sprite < 11
				@die_image[@die_sprite].draw(@player.x - $xcamera - 3, @player.y - $ycamera - 1, 35 + @p, 1, 1)
			elsif @player.facing == "left" and @die_sprite < 11
				@die_image[@die_sprite].draw(@player.x - $xcamera - 3 + 16, @player.y - $ycamera - 1, 35 + @p, -1, 1)
			end
			@dsp += 1
			if @dsp >= @dspmax
				@dsp = 0
		...
		@projectilearray.each do |proj|
			proj.draw(@player.x + (@player.width / 2), @player.y + (@player.height / 2))
		end

		if $debugmode == true
			@debug.draw("#{@player.rightcoll}", 0, 0, 35 + @p)
			@debug.draw("#{@player.leftcoll}", 0, 8, 35 + @p)
			@debug.draw("#{@player.bottcoll}", 0, 16, 35 + @p)
			@debug.draw("#{@player.y}", 0, 24, 35 + @p)
			@debug.draw("#{@player.yvelocity}", 0, 32, 35 + @p)
		end
		
		@tutorial.draw
		if @load == 2.0 and @currentlevel == 1.2
			if @player.y >= 688 and @tut_bools[3] == 0
				@tut_bools[3] = 1
				@tutorial.switch(70, 120, 0)
			end
			if @player.x >= 928 and @tut_bools[3] == 1
				@tut_bools[3] = 2
				@tutorial.switch(86, 120, 1)
			end
			if @player.x >= 1088 and @tut_bools[3] == 2
				@tut_bools[3] = 3
				@tutorial.switch(72, 120, 2)
			end
			if @player.aimcharge >= 25 and @tut_bools[3] == 3
				@tut_bools[3] = 4
				@tutorial.switch(72, 100, 3)
			end
			if $conditions[1] == 1
				@tut_bools[3] = 5
				@tutorial.switch(0, 0, -1)
			end
			temp = 0
			if @tut_bools[3] == 5
				$gems.count.times do |z|
					temp += $gems[z]
				end
			end
			if temp > 1 and @tut_bools[3] == 5
				@tut_bools[3] = 6
				@tutorial.switch(61, 120, 4)
			end
			if @tut_bools[3] == 7
				@tut_bools[3] = 8
				@tutorial.switch(0, 0, -1)
	...
	def use_proj(x, y, xv, yv, id)
		temp = false
		@projectilearray.each do |proj|
			if not temp
				if not proj.active and proj.id == id
					temp = true
					proj.active = true
					proj.x = x - (proj.width / 2)
					proj.y = y - (proj.height / 2)
					proj.xvelocity = xv
					proj.yvelocity = yv
				end
			end
	...
	def find_angle(x1, y1, x2, y2)
		return ((Math.atan2(y2.to_f - y1.to_f, x2.to_f - x1.to_f)) / Math::PI) * 180
	end
	...
	def solid_tile_collide_flyenemy(enemy, tile)
		if a_is_above_b(enemy, tile) and not a_hits_top_of_b(enemy, tile)
			if enemy.id == 0 or enemy.id == 1 and enemy.health > 0
				...
				enemy.bottcoll = false
			elsif enemy.id == 2 and enemy.health > 0
			end
			...
			if enemy.id == 0 or enemy.id == 1 and enemy.health > 0
				...
				if tile.canhit_b == 1 and enemy.yvelocity >= 0
					enemy.yvelocity *= -1
					enemy.y = tile.y - enemy.height
				end
			end
		...
		if a_is_below_b(enemy, tile) and not a_hits_bottom_of_b(enemy, tile)
			...
			if enemy.id == 0 or enemy.id == 1 and enemy.health > 0
				...
				if tile.canhit_t == 1 and enemy.yvelocity <= 0
					enemy.yvelocity *= -1
					enemy.y = tile.y + tile.height
				end
			end
		...
		if a_is_right_of_b(enemy, tile) and not a_hits_right_of_b(enemy, tile)
			...
			if enemy.id == 0 or enemy.id == 1 and enemy.health > 0
				...
				if tile.canhit_l == 1 and enemy.xvelocity <= 0
					enemy.xvelocity *= -1
					enemy.x = tile.x + tile.width
				end
			end
		...
		if a_is_left_of_b(enemy, tile) and not a_hits_left_of_b(enemy, tile)
			...
			if enemy.id == 0 or enemy.id == 1 and enemy.health > 0
				...
				if tile.canhit_r == 1 and enemy.xvelocity >= 0
					enemy.xvelocity *= -1
					enemy.x = tile.x - enemy.width
				end	
			end
